...
st.markdown("""
    ...
    .stButton>button { width: 100%; border-radius: 8px; background-color: #ff4b4b; color: white; }
...
@st.cache_resource
def load_yolo_model():
    # Looks for best.pt in the current directory
    return YOLO("best.pt")

try:
    model = load_yolo_model()
except Exception as e:
    st.error("⚠️ Could not find 'best.pt' in this directory. Please verify its location.")
    st.stop()

# 3. Sidebar controls for tweaking performance
...
uploaded_file = st.file_uploader("Choose a target image...", type=["jpg", "jpeg", "png"])
